...
│       │   └── test_individual.py
│       └── notebooks/
│           └── demo.ipynb         ← Notebook demo cá nhân nếu cần
├── team/
...
> Lưu ý: các đường dẫn `data/`, `src/`, `tests/` trong phần bài cá nhân được hiểu là nằm trong `individual/tuan/`.
...
- Có file `team/README.md` mô tả kiến trúc và phân công
...
Chấm bằng automated test suite (`pytest individual/tuan/tests/ -v`). Mỗi task có test riêng.

| Task | Nội dung | Điểm | Test |
|------|----------|------|------|
| 1 | Thu thập văn bản pháp luật (≥3 files tồn tại trong `individual/tuan/data/landing/legal/`) | 3 | `test_task1_*` |
| 2 | Crawl bài báo (≥5 files tồn tại trong `individual/tuan/data/landing/news/`) | 3 | `test_task2_*` |
| 3 | Convert markdown (files tồn tại trong `individual/tuan/data/standardized/`) | 4 | `test_task3_*` |
...
# Chạy từng task
pytest individual/tuan/tests/test_individual.py::TestTask1 -v
pytest individual/tuan/tests/test_individual.py::TestTask5 -v
```
